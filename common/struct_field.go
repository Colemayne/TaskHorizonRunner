	...
	if sf, ok := f.Value.(StructFieldValue); ok {
		if sf.IsBoolFlag() {
			flag := &cli.BoolFlag{
				...
				Usage: f.Usage,
				// Changed from -> EnvVar
				// Seems as if they wanted this to be an array instead.
				EnvVars: f.EnvVars,
			}
			return flag.String()
		} else {
...
func getStructFieldFlag(field reflect.StructField, fieldValue reflect.Value, ns []string) []cli.Flag {
	...
	flag := cli.GenericFlag{
		...
		// Changed from -> EnvVars: field.Tag.Get("env"),
		EnvVars: envArray,
	}
	// Changed from -> return []cli.Flag{StructFieldFlag{GenericFlag: flag}}
	// Had to change from 'struct literal' to a 'pointer to the struct literal'
	// Found the solution here https://github.com/urfave/cli/issues/459
